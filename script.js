...
function getNews(src) {
  for(let i = 0; i < src.length; i++) {
    ...
    xhr.open("GET", "https://newsapi.org/v1/articles?apiKey=ec755c2db8c74eeca16d9f3ff186ff96&source=" + src[i].id);
    xhr.send();
  }
}
...
function test_int() {
  ...
  if(int) {
    ...
    xhr.open("GET", "https://newsapi.org/v1/sources?language=en&category=" + int);  //수정
    xhr.send();
  }
}
